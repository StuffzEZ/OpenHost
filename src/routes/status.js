...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await query(
            ...
            (user_id, title, slug, description, show_last_deployment, show_uptime, is_public) 
...
router.put('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        await query(
            `UPDATE status_pages 
             SET title = $1, slug = $2, description = $3, show_last_deployment = $4, show_uptime = $5, is_public = $6 
             WHERE id = $7`,
            [title, slug, description, show_last_deployment, show_uptime, is_public, pageId]
        );

        // Update items (simplest is to delete and re-insert)
        ...
        res.json({ message: 'Status page updated successfully' });
    } catch (error) {
...
// Get items for a status page
router.get('/:id/items', authenticateToken, async (req, res) => {
    try {
        const result = await query('SELECT * FROM status_page_items WHERE status_page_id = $1', [req.params.id]);
        res.json({ items: result.rows });
    } catch (error) {
        logger.error('Get status items error:', error);
